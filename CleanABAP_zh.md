...
> [Avoid Encodings](AvoidEncodings.md) > 深入介绍了这样做的理由。
...
> [Function Groups vs. Classes](FunctionGroupsVsClasses.md) > 详细描述了两者的差异。
...
> [Enumerations](Enumerations.md) > 描述了常见的枚举模式> 并讨论了它们的优缺点。
> 
...
> [Interfaces vs. abstract classes](InterfacesVsAbstractClasses.md) 对此做了一些详细比较。
...
> [接口与抽象类](InterfacesVsAbstractClasses.md)描述了为什么这也适用于覆盖继承方法的类。
...
> 这与 [ABAP Programming Guidelines](https://help.sap.com/doc/abapdocu_751_index_htm/7.51/en-US/abenexception_category_guidl.htm) 一致，但与 [Robert C. Martin 所著的 _Clean Code_]（其中建议优先使用不可控异常）相矛盾；[异常](Exceptions.md)说明了其中的原因。
...
> [大写字母与小写字母](UpperVsLowerCase.md)解释了为什么我们没有为关键字的大小写提供明确的指导。
更多信息参阅 [Robert C. Martin 所著的 _Clean Code_] 中的 _Chapter 5: Formatting: Team Rules_。
